...
echo Compilation des sous-packages (model, controller, util, test, admin)...

if exist "testFramework\com\testframework\model\*.java" (
    echo Compilation du package model...
    javac --release 17 -classpath "build\classes" -d "build\classes" testFramework\com\testframework\model\*.java
    if errorlevel 1 (
        echo Erreur de compilation du package model!
        pause
        exit /b 1
    )
)
...
if exist "testFramework\com\testframework\util\*.java" (
    javac --release 17 -classpath "build\classes" -d "build\classes" testFramework\com\testframework\util\*.java
)

if exist "testFramework\com\testframework\test\*.java" (
    javac --release 17 -classpath "build\classes" -d "build\classes" testFramework\com\testframework\test\*.java
)

if exist "testFramework\com\testframework\admin\*.java" (
    javac --release 17 -classpath "build\classes" -d "build\classes" testFramework\com\testframework\admin\*.java
)

REM Compilation de la classe Main
echo Compilation de la classe Main...
javac --release 17 -classpath "build\classes" -d "build\classes" testFramework\com\testframework\Main.java
